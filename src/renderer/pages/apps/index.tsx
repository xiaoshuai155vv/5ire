            ...
            {apps.map((app:IAppConfig) => (
              ...
              >
                <div className="flex flex-col items-start justify-center">
                  <div className="app-icon mb-3"><Image src={app.icon} width={60} height={60} /></div>
                  <div className="text-primary-500 text-lg font-medium">{app.name}</div>
                  <div className="text-color-secondary text-xs mt-1">{app.description}</div>
                </div>
              </div>
            ))}
          </div>
